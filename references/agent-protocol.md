...
## 5.1 章节标题硬约束(phase3-part 全体适用)

★ **所有 `## §X ...` 章节标题必须与 `assets/templates/report-skeleton.md` 逐字一致,禁止改写/缩写/自由发挥。**

各 part 必须使用的标题(从 skeleton 原样复制):

| Part | 标题(逐字复制) |
|------|---------------|
| part1 | `## §一 执行摘要` / `## §二 事实评分总览（10 维度）` / `## §三 快速筛选（致命看空条款 - 6 项）` |
| part2 | `## §四 公司基本面` / `## §五 行业与竞争格局` |
| part3 | `## §六 10 维度详细证据` / `## §七 网络舆情与市场情绪` / `## §八 可比公司对标（v4.4 — Read \`peer_analysis.md\`）` |
| part4 | `## §九 估值与回报模拟` / `## §十 投资回报测算（与 §九 共用情景）` / `## §十一 定性判断（3 框架，v4.1 — 无打分）` |
| part5 | `## §十二 信息缺口与尽调优先级` / `## §十三 数据可审计性（时效性 + 来源 3 类分组）` |

**违反后果**: Phase 6 `anti_lazy_lint` Rule 4 会 FAIL,触发修正循环浪费 token。标题真理来源始终是 `report-skeleton.md`,本表仅为快速参考。
