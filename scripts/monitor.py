...
data to identify material changes and trigger falsification checks on
insight cards (if any exist from legacy Phase 5 reports).

NOTE: Phase 5 (variant perception / insight cards) was removed in v5.1.4.
The insight-checking code is retained for backward compatibility with older
reports that contain phase5-variant-perception.md files.
...
def _find_phase5_file(company_dir: Path) -> Path | None:  # LEGACY — Phase 5 removed in v5.1.4
...
def _format_summary(result: MonitorResult) -> str:
    ...
    lines += [
        ...
        "---",
        "",
        "## §2 洞察证伪检查（LEGACY — Phase 5 removed in v5.1.4）",
        "",
    ]
    ...
    if result.conclusion == "重大修订":
        lines.append("⚠️ 多项关键指标变化幅度超过 10%。**建议立即重跑完整分析**。")
    elif result.conclusion == "建议复评":
        lines.append("🟡 检测到部分重大变化。**建议人工复核相关章节**。")
    else:
        lines.append("✅ 所有指标稳定。**维持现有投资结论**。")
