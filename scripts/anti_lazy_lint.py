...
# v5.1.4: 跨文件引用白名单 (章节重排后)
# §十二 = 信息缺口 (旧 §十四) / §十三 = 数据可审计性 (旧 §十五)
SECTION_WHITELIST: dict[str, set[str]] = {
    "§十三": {"audit_report.md", "metrics.json", "phase1-data.md", "phase2-documents.md"},
}

# ============================================================================
# Rule 2: 各章节最低字符数 (中文+字母+数字, 排除标题行)
# 阈值 calibration 来自 v4.6 实测: 摘要式章节 (§二/§三/§十/§十二) 给 150-200,
...
MIN_SECTION_CHARS = {
    ...
    "§十":  200,  # 回报简表
    "§十一": 200, # 定性 3 框架
    "§十二": 100, # 信息缺口表 (v5.1.4: 旧 §十四)
    "§十三": 100, # 数据来源 (v5.1.4: 旧 §十五)
}
...
def rule_4_title_byte_exact(md_text: str) -> RuleResult:
    ...
    detail = (
        f"{len(findings)} 处不匹配"
        if not passed
        else f"13/13 章节标题与骨架字节一致 (去括号注释后)"
    )
