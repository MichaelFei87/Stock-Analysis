...
修复 v4.6 的 3 个问题:
1. MD 章节 vs HTML 13 个固定 placeholder + extra_sections 溢出区
...
4. 前 13 section 填入 base.html 的 section_1..13 固定占位(锚点 id 不变)
5. 第 14+ section 追加到 extra_sections 占位(附录等)
...
def build_html(
    ...
    # 10. 额外 section(第 14+)追加到 extra_sections
    # v4.7 fix #6: extra_sections 占位必须存在,否则第 14+ 章静默丢失
    if "<!-- PLACEHOLDER: extra_sections -->" not in html:
        raise AssertionError(
            "base.html 缺 extra_sections 占位 — 第 14+ 章节会静默丢失 (v4.7 fix #6)"
        )
    extra_parts = []
    for idx, (title, body_md) in enumerate(sections[13:], start=14):
        section_id = f"extra-{idx - 13}"
        extra_parts.append(
