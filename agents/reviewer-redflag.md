...
### 前置检查: audit 完整性 meta-check (★ v5.1.6 新增)

读取 `audit_report.md` §1 框架执行状态，统计:
- `有结果框架数` = 包含"✅"的行数
- `数据不足框架数` = 包含"⚠️ 数据不足"的行数

| 条件 | 判定 |
|---|---|
| 有结果框架 ≥ 4 | 正常，继续红旗闭环检查 |
| 有结果框架 1-3 | ⚠️ 标注"audit 覆盖不足（{N}/12 框架有结果）"，继续检查已有红旗 |
| **有结果框架 = 0**（全部数据不足） | **FAIL** — audit 脚本未能正常工作，0 红旗不可信。FIX: `[FIX-META] audit 11+ 框架全部返回"数据不足"，疑似数据格式/fiscal year 不兼容，需人工确认 financial_audit.py 是否正确识别了年报行` |
