...
2. HTML模板指南可用：`references/html-template-guide.md`
...
- §一~§十三 章节标题被 LLM 重命名(如 §四 公司基本面 → §四 业务概况)
...
| # | 规则 | 阈值 | 例外白名单 |
|:---:|------|------|---------|
| 1 | 外链引用扫描 (详见 xxx.md / 见 phaseX.md / [xxx](xxx.md)) | 命中 = 0 | §十三 允许引用 audit_report.md 等 |
...
| 12 | **定性判断完整性** | §十一 定性判断 3 框架是否齐全？结论是否严格 3 档（看多/看空/中性-分歧）？ |
| 13 | **信息缺口质量** | §十二 信息缺口与尽调优先级表格是否至少 3 条？每条是否有明确状态（✅/⚠️/❌）？是否列出了查询路径？ |
| 14 | **评分独立性 + 理由列删除（v4.1）** | §二 评分总览是否**只 4 列**（维度/权重/分数/加权分），**无"关键理由"列**？10 维度分数是否未因洞察被调整？**无"定性修正系数 -0.5"这种打分式写法**？ |
| 15 | **数据来源分组** | §十三 数据可审计性是否按 3 类分组（Tushare API / PDF 原文 / WebSearch）？ |
...
| **17** | **缺口补查闭环（v3 新增）** | §十二 "信息缺口与尽调优先级"表格中每一项是否有**明确状态**（✅已解决 / ⚠️部分 / ❌未找到）？是否列出了 Phase 1 尝试的查询路径？**若有 ❌ 未找到的条目，Part D 是否已执行补查？** |
...
| **21** | **HTML 资产加载（v4.3 新增）** | Phase 6 Part B 生成 HTML 是否**从 `assets/html/base.html` 加载骨架**？是否**内联了 `assets/html/styles.css` 完整内容**（`grep -c '^\s*--c-' *.html` ≥16 个 CSS 变量）？是否使用了 `assets/html/components.html` 中的 9 个组件 class（`grep -c 'class="(score-ring|dimension-bar|scenario|expected-return|team-card|risk-item|timeline|sentiment-bar|valuation-range)"'` ≥8）？是否 13 个 `<div class="section"` 对应 §一～§十三 且 id 属性正确？**禁止 Claude 自写 CSS 变量或组件 class**。 |
| **22** | **Executive Summary 7 字段 schema（v4.3 新增）** | §一 执行摘要是否严格按 `assets/templates/exec-summary-schema.md` 的 7 固定字段展开？字段名与顺序字节一致（一句话结论 / 估值锚 / 综合评分 / 三大风险 / 三大机会 / 核心非共识观点 / 投资方向综合判定）？**是否出现禁用字段**：综合评级 / 量化分 / 定性修正 / 调整后分 / 建议仓位 / 尽调优先级（出现任一则不通过）？**章节标题 13 个 `## §` 是否与 `assets/templates/report-skeleton.md` 字节一致**？ |
...
4. 按 ^## 切 MD section,**前 13 填固定 placeholder,第 14+ 追加到 extra_sections** (避免 v4.6 之前的"附录丢失"bug)
...
Step 4: 逐章节填充 13 个 <!-- PLACEHOLDER: section_N_xxx --> 占位:
...
- ❌ **禁止"概括/合并/简化"** MD 章节 — 13 个 section 必须一一对应

**HTML 13 章节要求**（严格与 `assets/templates/report-skeleton.md` 对齐）:
...
| 12 | 信息缺口与尽调 | `## §十二 信息缺口与尽调优先级` | `gaps` |
| 13 | 数据可审计性 | `## §十三 数据可审计性（时效性 + 来源 3 类分组）` | `sources` |
...
补查结果**必须同步更新**以下所有位置（不仅是 §十二）：

| 补查结果 | 必须更新的章节 |
|---------|---------------|
| 数据直接涉及 Q3 亏损归因 | §六 盈利质量维度 |
| 数据涉及分业务毛利/收入 | §四 财务趋势表 + §八 可比对标 |
| 数据涉及子公司/参股公司 | §四 基本面分析 |
| 数据涉及高管/股权激励 | §六 维度 3 团队 + §十 条款分析 |
| 数据涉及估值（分红/回购/股东户数） | §二 评分总览 + §九 估值分析 |

**强制反写校验**：生成最终报告前，对每个补查成功条目执行 grep：
```bash
grep -l "{缺口项关键词}" output/{company}/{company}-analysis-*.md
```
若只在 §十二 出现一次，而其他应引用的章节里没引用 → 视为"孤岛化错误"，必须补写。

### Part D 自检（保存 phase6-review-log.md 前必须通过）

- [ ] 所有状态 ⚠️ / ❌ 的缺口都执行了 5 步补查？
- [ ] 每一步的尝试结果（✅/⚠️/❌/⏭️）都详细记录在登记表？
- [ ] 补查成功的数据已反写到 **所有相关章节**（不只 §十二）？
- [ ] 补查失败的条目标注 "信息可得性：低 / 原则上不可得" 且说明原因？
- [ ] **"部分解决"条目**都写明"还需要什么数据才能升级为已解决"？
- [ ] §十二 缺口条目数 ≥ 3（若 Phase 1/Phase 3 少于 3，Part D 已触发反向扫描）？
...
- [ ] **22 项**审核清单全部执行且有记录？
- [ ] HTML 包含全部 13 章节？
- [ ] HTML §十二 信息缺口表格完整且有状态标注？
- [ ] HTML §十一 定性判断**无打分数字**，用逻辑三段式？
- [ ] HTML 评分总览保持单列（无"调整后"分数）？
- [ ] HTML 与 MD 内容一致？
